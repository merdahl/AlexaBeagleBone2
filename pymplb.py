...
"""
pymplb (PYthonMPLayerBingings) is a library that can be used to play media using
an external MPlayer process.

The library runs the MPlayer binary in slave mode as a subprocess and then sends
slave-mode commands to the process.  Commands are mapped to class methods and 
properties to class properties (by default prefixed with 'p_').  Commands are
discovered at runtime and thus these bindings should automatically also support
any new commands added to MPlayer in the future.
...
def make_mplayer_class(mplayer_bin='mplayer', method_prefix='', property_prefix='p_'):
	"""
	Construct a MPlayer class which user mplayer_bin as the player binary and 
	prepends the given prefixes to property and method names.
	Prefixes are needed because some properties and methods have the same name.
	You only need to construct a new class if the default values are not 
	suitable (i.e. mplayer is not in your path, or some new commands have been 
	introduced that conflict with the default prefixes.
	"""
	...
	class _MPlayer(object): #pylint: disable-msg=R0903
		"""
		This is the main class used to play audio and video files by launching 
		mplayer as a subprocess in slave mode.

		Slave mode methods can be called directly (e.g. x.loadfile("somefile)") 
		while properties are prefixed to avoid name conflicts between methods 
		and properties (e.g. x.p_looping = False).
		
		Available methods and properties are determined at runtime when the 
		class is instantiated. All methods and properties are
		type-safe and properties respect minimum and maximum values given by 
		mplayer.
		"""
		_arg_types = {'Flag':type(False), 'String':type(''), 'Integer':type(0), 
					  'Float':type(0.0), 'Position':type(0.0), 'Time':type(0.0)
					  } # Mapping from mplayer -> Python types
		...
		def __cleanup(self):
			"""
			Method that kills the MPlayer subprocess when the application using 
			the library exits
			"""
			os.killpg(os.getpgid(self.__player.pid), signal.SIGTERM)
		...
		def _add_methods(cls, mplayer_bin):
			...
			def cmd(name, argtypes, obligatory, player, *args, **kwargs):
				...
				if len(args) < obligatory:
					raise TypeError('TypeError: %s() takes at least %d arguments (%d given)'%
							(name, obligatory, len(args)))
				if len(args) > len(argtypes):
					raise TypeError('TypeError: %s() takes at most %d arguments (%d given)'%
							(name, len(argtypes), len(args)))
				for i in range(len(args)):
					if type(args[i]) != argtypes[i]:
						raise TypeError('Argument %d of %s() has type %s, should be %s'%
								(i, name, type(args[i]).__name__, argtypes[i].__name__))
		...
		def _add_properties(cls, mplayer_bin):
			"""
			Discover which properties MPlayer understands and add them as class 
			properties
			"""
			def get_prop(name, prop_type, islist, self):
				"""
				Function which calls the get_property method to get the property 
				value and does some type checking
				"""
			...
			def set_prop(name, prop_type, islist, prop_min, prop_max, self, value):
				"""
				Function which calls the set_property method to set the property
				value and does some type checking
				"""
				if islist:
					for elem in value:
						if type(elem) != prop_type:
							raise TypeError('TypeError: Element %s has wrong type %s, not %s'%
									(elem, type(elem).__name__, prop_type))
						if prop_min != None and elem < prop_min:
							raise ValueError('ValueError: Element %s must be at least %s'%
									(elem, prop_min))
						if prop_max != None and elem > prop_max:
							raise ValueError('ValueError: Element %s must be at most %s'%
									(elem, prop_max))
					value = ','.join([str(elem) for elem in value])
				else:
					if type(value) != prop_type:
						raise TypeError('TypeError: %s has type %s, not %s'%
								(name, prop_type.__name__, type(value).__name__))
					if prop_min != None and value < prop_min:
						raise ValueError('ValueError: %s must be at least %s (>%s)'%
								(name, prop_min, value))
					if prop_max != None and value > prop_max:
						raise ValueError('ValueError: %s must be at most %s (<%s)'%
								(name, prop_max, value))
			...
			for line in player.stdout:
				...
				setattr(cls, cls._property_prefix+name, property(getter, setter, 
						doc='Property of type %s in range [%s, %s].'%
						(prop_type.__name__, prop_min, prop_max)))
